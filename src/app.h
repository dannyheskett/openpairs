#ifndef OPENPAIRS_APP_H
#define OPENPAIRS_APP_H

// iOS entry points. On iOS, UIKit owns main() and the run loop, so the app shell
// (ios/ios_main.mm) drives the game via these instead: op_app_init() once at
// startup, then op_app_frame() every CADisplayLink tick. Implemented in main.c
...
void op_app_frame(void);
#ifdef __cplusplus
}
#endif

#endif // OPENPAIRS_APP_H
