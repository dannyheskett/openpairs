...
openpairs ships for **iPhone and iPad** (`UIDeviceFamily = [1, 2]` in
...
- A press has to be held past a couple of frames. 60ms is silently dropped;
  120ms is reliable.
- The card face is a near-white within a few counts of the title text, so the
  grid is found by looking for card BACKS on a freshly dealt board. Both back
  colours must count, generously: the antialiased pixels between the back and
  its lattice pattern otherwise chop one card into several runs.
- The script plays the board honestly — turning cards up, remembering what it
  saw, and matching pairs — so the frames show a real game rather than a posed
  one.
