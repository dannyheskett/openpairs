...
# OPENPAIRS_VERSION explicitly; for local `make dist` it derives from the
...
# repo's release.yml is byte-identical. OPENPAIRS_VERSION still works as an explicit
...
OPENPAIRS_VERSION ?= $(RELEASE_VERSION)
VERSION_SLUG       := build-$(OPENPAIRS_VERSION)
...
OUT         := build/openpairs
OUT_RELEASE := build/openpairs-release
...
OUT_WIN64 := build/openpairs-x64.exe
OUT_WIN32 := build/openpairs-x86.exe
...
# native_app_glue into libopenpairs.so, then package + sign an APK with
...
# only exists when this flag is present. Without it, dlopen of libopenpairs.so
...
ANDROID_LIB      := $(ANDROID_APK_DIR)/lib/$(ANDROID_ABI)/libopenpairs.so
ANDROID_APK      := build/openpairs.apk
ANDROID_KEYSTORE ?= build/debug.keystore

# One tiny Java class (OpenpairsActivity, a NativeActivity subclass that hides
...
ANDROID_JAVA_SRC := android/java/com/danheskett/openpairs/OpenpairsActivity.java
...
$(ANDROID_DEX): $(ANDROID_JAVA_SRC)
	...
	$(ANDROID_SDK_BT)/d8 --min-api $(ANDROID_API) --lib $(ANDROID_JAR) \
	    --output build/dex build/java-classes/com/danheskett/openpairs/*.class
...
$(ANDROID_KEYSTORE):
	...
	keytool -genkeypair -keystore $@ -storepass android -keypass android \
	    -alias openpairs -keyalg RSA -keysize 2048 -validity 10000 \
	    -dname "CN=openpairs, O=openpairs, C=US"
...
# sideload APK target: same libopenpairs.so, different packaging + a real
...
# ---------------------------------------------------------------------------
ANDROID_AAB        := build/openpairs.aab
...
$(ANDROID_AAB): $(ANDROID_LIB) $(ANDROID_DEX) $(BUNDLETOOL) $(PLAY_KEYSTORE) \
	...
	cp $(ANDROID_LIB) build/aab/module/lib/$(ANDROID_ABI)/libopenpairs.so
...
# Outputs build/web/openpairs.{html,js,wasm} with the shell in web/shell.html.
# ---------------------------------------------------------------------------
...
WEB_OUT     := $(WEB_OUT_DIR)/openpairs.html
...
web-serve: $(WEB_OUT)
	@echo "openpairs: http://localhost:8080/openpairs.html"
...
# ---------------------------------------------------------------------------
IOS_MIN        ?= 15.0
IOS_APP_NAME   := Openpairs
IOS_BUNDLE_ID  := com.danheskett.openpairs
...
IOS_BUILD_NUMBER ?= $(OPENPAIRS_VERSION)
...
$(IOS_IPA): $(IOS_DEPS)
	...
	cd build/ios-device && rm -f ../openpairs.ipa && zip -qr ../openpairs.ipa Payload
	@if [ -n "$(IOS_SIGN_IDENTITY)" ]; then \
...
#   test_layout — the two board layouts, compiled -DPLATFORM_IOS so op_types.h
#                 supplies the geometry types without raylib, and with
#                 OP_FIXED forced on so both can be exercised in one binary
...
dist-linux: release
	@rm -rf $(STAGING)/linux && mkdir -p $(STAGING)/linux/openpairs-$(VERSION_SLUG) $(DIST)
	cp $(OUT_RELEASE) $(STAGING)/linux/openpairs-$(VERSION_SLUG)/openpairs
	cp $(DOCS) $(STAGING)/linux/openpairs-$(VERSION_SLUG)/
	(cd $(STAGING)/linux && tar -czf ../../../$(DIST)/openpairs-$(VERSION_SLUG)-linux-x86_64.tar.gz openpairs-$(VERSION_SLUG))

dist-windows: $(OUT_WIN64) $(OUT_WIN32)
	@rm -rf $(STAGING)/win && mkdir -p $(DIST) \
	    $(STAGING)/win/openpairs-$(VERSION_SLUG)-x64 \
	    $(STAGING)/win/openpairs-$(VERSION_SLUG)-x86
	cp $(OUT_WIN64) $(STAGING)/win/openpairs-$(VERSION_SLUG)-x64/openpairs.exe
	cp $(DOCS) $(STAGING)/win/openpairs-$(VERSION_SLUG)-x64/
	cp $(OUT_WIN32) $(STAGING)/win/openpairs-$(VERSION_SLUG)-x86/openpairs.exe
	cp $(DOCS) $(STAGING)/win/openpairs-$(VERSION_SLUG)-x86/
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/openpairs-$(VERSION_SLUG)-windows-x64.zip openpairs-$(VERSION_SLUG)-x64)
	(cd $(STAGING)/win && zip -qr ../../../$(DIST)/openpairs-$(VERSION_SLUG)-windows-x86.zip openpairs-$(VERSION_SLUG)-x86)

dist-mac: $(OUT_MAC)
	@rm -rf $(STAGING)/mac && mkdir -p $(STAGING)/mac/openpairs-$(VERSION_SLUG) $(DIST)
	cp $(OUT_MAC) $(STAGING)/mac/openpairs-$(VERSION_SLUG)/openpairs
	codesign --force --sign - $(STAGING)/mac/openpairs-$(VERSION_SLUG)/openpairs
	cp $(DOCS) $(STAGING)/mac/openpairs-$(VERSION_SLUG)/
	(cd $(STAGING)/mac && zip -qr ../../../$(DIST)/openpairs-$(VERSION_SLUG)-macos-universal.zip openpairs-$(VERSION_SLUG))

# The APK is a self-contained installable, so it ships as-is (versioned name).
dist-android: $(ANDROID_APK)
	@mkdir -p $(DIST)
	cp $(ANDROID_APK) $(DIST)/openpairs-$(VERSION_SLUG)-android-arm64.apk

# The Play upload artifact: the signed App Bundle (see the android-play target).
dist-android-play: $(ANDROID_AAB)
	@mkdir -p $(DIST)
	cp $(ANDROID_AAB) $(DIST)/openpairs-$(VERSION_SLUG)-android.aab

dist-ios: $(IOS_IPA)
	@mkdir -p $(DIST)
	cp $(IOS_IPA) $(DIST)/openpairs-$(VERSION_SLUG)-ios-arm64.ipa

# The web build ships as a zip of the HTML/JS/WASM (serve over http to play).
dist-web: $(WEB_OUT)
	@rm -rf $(STAGING)/web && mkdir -p $(STAGING)/web/openpairs-$(VERSION_SLUG)-web $(DIST)
	cp $(WEB_OUT_DIR)/openpairs.html $(WEB_OUT_DIR)/openpairs.js $(WEB_OUT_DIR)/openpairs.wasm \
	    $(STAGING)/web/openpairs-$(VERSION_SLUG)-web/
	cp $(DOCS) $(STAGING)/web/openpairs-$(VERSION_SLUG)-web/
	(cd $(STAGING)/web && zip -qr ../../../$(DIST)/openpairs-$(VERSION_SLUG)-web-wasm.zip openpairs-$(VERSION_SLUG)-web)
